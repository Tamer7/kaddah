...
class ProductsController extends VoyagerBaseController
{
    ...
    public function show(Request $request, $id)
    {
        ...
        $features = $dataTypeContent->features()->get();
        $related = $dataTypeContent->related()->get();
        //$capacities = $dataTypeContent->capacities()->get();
        
        return Voyager::view($view, compact('dataType', 'dataTypeContent', 'isModelTranslatable', 'isSoftDeleted', 'features', 'related'));
    }
    ...
    public function edit(Request $request, $id)
    {
        ...
        $relatedproducts = RelatedProduct::where('product_id', $id)->pluck('related_id')->toArray();
        
        $products = Product::all();

        return Voyager::view($view, compact('dataType', 'dataTypeContent', 'isModelTranslatable', 'features', 'products', 'relatedproducts'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        //Delete
        $query = Feature::where('product_id', '=', $id);
        
        if($request->feature_id && count($request->feature_id) > 0) {
            $query->whereNotIn('id', $request->feature_id);
        } 
        $query->delete();

        if($request->feature_name && count($request->feature_name) > 0) {
            foreach ($request->feature_name as $key => $feature) {
                if($feature && isset($request->feature_value[$key]) && $request->feature_value[$key]) {
                    if(isset($request->feature_id[$key]) && is_numeric($request->feature_id[$key])) {
                        
                        $featureObj = Feature::where('product_id', '=', $id)->where('id', $request->feature_id[$key])->first();
                        $featureObj->name = $feature;
                        $featureObj->value = $request->feature_value[$key];
                        $featureObj->save();
                    } else {
                        // dd($features);
                        array_push($features, [
                            "product_id" => $data->id,
                            "name" => $feature,
                            "value" => $request->feature_value[$key]
                        ]);
                    }
                }
            }
        ...
        if(count($features) > 0) {
            Feature::insert($features); #add laravel insert query
        }
    ...
    public function create(Request $request)
    {
        ...
        $relatedproducts = RelatedProduct::where('product_id', $request)->pluck('related_id')->toArray();
    ...
    public function store(Request $request)
    {
        ...
        //     // do the stuff for product and related product input validations
        //     'product_id'  =>  'required',
        //     'related_id' =>  'required',
        // ]);
